...
def compute_smoothness(cycle, G):
    ...
    score = smoothness_base * 100 - sharp_penalty
    score = max(0.0, score)
...
def load_graph(path):
    ...
    if G.number_of_nodes() > 0:
        ...
        print(f"节点坐标范围:")
        print(f"  经度: {min_x:.6f} ~ {max_x:.6f}")
        print(f"  纬度: {min_y:.6f} ~ {max_y:.6f}")
        print(f"  中心点: ({center_x:.6f}, {center_y:.6f})")
        print(f"  💡 建议起始点: {center_x:.6f}, {center_y:.6f}")
    else:
        print("⚠️ 图中没有节点，请检查 GraphML 文件是否包含有效数据")
        print("   提示：真实数据请用 build_road_network.py 重新生成并扩大裁剪范围")
        print("   模拟数据请用 generate_osm_like_network.py 重新生成")
...
def main():
    ...
    print(f"总环数: {total}")
    if total > 0:
        print(f"最高顺畅度: {max(c['smoothness'] for c in cycle_data)}")
...
<head>
    ...
    <style>
        ...
        .control-panel {{
            ...
            max-height: 80%; overflow-y: auto; width: 380px;
        }}
        .control-panel h4 {{ margin: 0 0 5px 0; }}
        .control-panel select, .control-panel input {{ width: 100%; margin-bottom: 8px; }}
...
<body>
    ...
    <div class="control-panel">
        ...
        <div>
            ...
            <select id="sortSelect">
                ...
                <option value="distance">按距起点距离 (近->远)</option>
                <option value="composite">综合排序 (顺畅度 - 惩罚系数 * 距离)</option>
            </select>
        </div>
        <div id="compositeControl" style="display:none;">
            <label>距离惩罚系数 (越大越看重近):</label>
            <div class="slider-container">
                <span>0</span>
                <input type="range" id="weightSlider" min="0" max="5" step="0.1" value="0.5">
                <span>5</span>
            </div>
            <span id="weightValue">当前: 0.5</span>
        </div>
    ...
    <script>
        ...
        function updateDisplay() {{
            ...
            document.getElementById('weightValue').innerText = '当前: ' + weight;
            ...
            if (sortBy === 'length') {{
                ...
                sorted.sort((a,b) => {{
                    var scoreA = a.smoothness - weight * a.min_dist_km;
                    var scoreB = b.smoothness - weight * b.min_dist_km;
        ...
        function toggleCompositeControl() {{
            var sortBy = document.getElementById('sortSelect').value;
            var compositeDiv = document.getElementById('compositeControl');
            compositeDiv.style.display = (sortBy === 'composite') ? 'block' : 'none';
            updateDisplay();
        }}

        document.getElementById('sortSelect').addEventListener('change', toggleCompositeControl);
        document.getElementById('weightSlider').addEventListener('input', function() {{
            if (document.getElementById('sortSelect').value === 'composite') updateDisplay();
            document.getElementById('weightValue').innerText = '当前: ' + this.value;
        }});
        toggleCompositeControl();
    </script>
</body>
</html>
    """
    ...
    output_html = os.path.join(OUTPUT_DIR, "cycles_map_composite.html")
